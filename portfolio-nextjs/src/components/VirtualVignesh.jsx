...
const VirtualVignesh = () => {
    ...
    const smartNavigate = useCallback((target) => {
        ...
        if (sectionInfo) {
            if (sectionInfo.isHomepageSection) {
                ...
                router.push(sectionInfo.route);
                setLastAction(`Navigated to ${target}`);
            }
        } else {
            // Unknown section - try navigating as a route
            router.push(`/${targetLower}`);
            setLastAction(`Navigated to ${target}`);
        }
    }, [router]);
    ...
    const quickActions = [
        ...
        { label: "💻 Skills", action: "What are your skills?" },
